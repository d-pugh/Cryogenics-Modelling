...
f = 12
R = 8.314               # J/(mol K)
n_max = 0.1               # maximum adsorbate capacity (mol)
E = 1000.0              # characteristic adsorption energy (J/mol)
T_STP = 273.15         # K
P_STP = 101325         # Pa
L3 = 25              # latent heat of vaporization (J/mol)
V_mol_liq3 =  37.87e-6 # m^3/mol, molar volume of liquid He-3 at 0.3K and SVP
...
file_path = os.path.join(os.path.dirname(__file__), 'vapour_pressure_He3.txt')
...
def P0_helium(T):
    """Saturation pressure of He-3 [Pa]. Works for scalars and arrays."""
...
def total_charge(Qdot, hold_time, P, T_cond, T_evap, V_evap, V_pump):
    n_l = scf.calculate_n_l(Qdot, hold_time, L3)    # assume all liquid evaporates
    n_e = scf.calculate_n_e(P, T_evap, V_evap, n_l, V_mol_liq3)
...
if __name__ == "__main__":
    ...
    t_hold = 6
    P = P0_helium(T_evap)
    n_l = scf.calculate_n_l(Qdot, t_hold, L3)
    V_l = n_l * V_mol_liq3 
    n_e = scf.calculate_n_e(P, T_evap, V_evap, n_l, V_mol_liq3)
    n_t = scf.calculate_n_t(P, r, l_pump, lambda z: scf.T_z(z, T_cond, T_evap, l_pump))
    n_p = scf.calculate_n_p(P, T_cond, V_pump, n_max, E, P0_helium(T_cond))
    ...
    print("=== He-3 Sorption Cooler Charge Calculation ===")
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('he3_gas_charge_vs_hold_time1.png', dpi=600)
    plt.show()
    plt.close()
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('he3_gas_charge_vs_hold_time2.png', dpi=600)
    plt.show()
    plt.close()
